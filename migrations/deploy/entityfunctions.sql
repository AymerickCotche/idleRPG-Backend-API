...
CREATE OR REPLACE FUNCTION getAllEntities()
	RETURNS TABLE (
		...
		WITH entity_att AS
			...
			rewards_items AS
			(SELECT entity_reward_item.id, entity_reward_item.quantity, entity_reward_item.entity_id, entity_reward_item.drop_rate, item.name AS item_name FROM entity_reward_item
